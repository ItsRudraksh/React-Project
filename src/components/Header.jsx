...
const Header = () => {
  ...
  // Media query to check if the screen width is small or medium
  const isSmallOrMedium = useMediaQuery({ query: "(max-width: 1023px)" });

  return (
    // Header container with fixed positioning, semi-transparent background, and blur effect
    <div className="w-full h-24 bg-[rgba(0,0,0,0.4)] px-4 items-center flex justify-between border-b-2 border-b-[#7b7b7b] fixed top-0 z-10 backdrop-blur-sm sm:h-20 md:h-20">
      {/* Logo section */}
      <div className="w-1/5 flex items-center justify-end lg:justify-center">
        <Link
      ...
      {/* Conditionally render MobileHeader or standard navigation based on screen size */}
      {isSmallOrMedium ? (
        <MobileHeader
          navLinks={navLinks}
          iconLinks={iconLinks}
